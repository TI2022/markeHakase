class ReservationValidator < ActiveModel::EachValidator
  ...
  def validate_each(record, attribute, value)
    ...
    if working_staff.count >= store.working_staff
      record.errors.add("稼働スタッフは#{store.working_staff}人です。") if working_staff.count >= store.working_staff # 同時刻予約はスタッフ稼働人数まで
    elsif not_own_periods.present?
      record.errors.add('予約時間に重複があります') if not_own_periods.present?
    end
